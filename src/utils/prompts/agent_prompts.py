...
1️⃣ **УТОЧНЕНИЕ** (первый ход):
   - Спроси: "Это про задачу [краткое описание из backend_tools]?"
   - Если не та задача — попроси показать условие
...
5️⃣ **ЗАВЕРШЕНИЕ**:
   ...
   ❌ Если не получается после 5 подсказок:
