...
3) "task_problems"
Выбирается для любых проблем с уроками или заданиями, а также для помощи с решением задач:
- не открывается;
- ошибка или баг в упражнении;
- не засчитало выполнение;
- не получается пройти задание, зависает, не грузится;
- просьба помочь с задачей/есепом/примером по математике;
- вопросы про текущую задачу: "какая задача?", "знаешь ли задачу?", "это какой есеп?";
- "не могу решить", "не получается задача", "есеп шықпай жатыр", "көмек керек есеп";
- любые вопросы, связанные с решением математических задач или упражнений.
...
– благодарности и прощания ("спасибо", "рахмет", "пока", "сау бол") - ТОЛЬКО если это завершение разговора, а не продолжение решения задачи.

ВАЖНО про продолжение диалога:
- Если в контексте идёт решение задачи (task_problems) и ученик отвечает на вопрос бота - это task_problems, НЕ neutral.
- Если ученик благодарит после решения задачи - это neutral (завершение).
- Если ученик посреди решения задачи спрашивает про заморозку/кэшбек - это соответствующий новый интент.
...
ВАЖНО - ИНТЕРПРЕТАЦИЯ В КОНТЕКСТЕ:
- Всегда интерпретируй сообщение ученика в контексте ТЕКУЩЕГО диалога.
- Если идёт обсуждение математической задачи, числа и термины в сообщении относятся к этой задаче.
- Короткие ответы (числа, "да/нет", "иә/жоқ") - это скорее всего ответы на вопросы бота, а не новые запросы.
...
subintent_task_problems_prompt = """Ты - строгий и точный классификатор субинтентов по теме «проблемы с заданием».
Твоя задача - выбрать ровно ОДИН субинтент из фиксированного списка и вернуть JSON-словарь строго с двумя ключами:
{"subintent":"...", "confidence":0.xx}
...
1) "task_problems"
Выбирается, если пользователь просит помощи в решении задачи или у него не получается её выполнить:
- «не могу решить», «помоги с задачей», «есеп шықпай жатыр»;
- вопросы о подсказке, объяснении шагов решения;
- просьба разобрать конкретный пример или математическое упражнение.

2) "change_task"
Выбирается, если пользователь хочет сменить задание или считает его некорректным:
- «смените задание», «хочу другое задание», «задание неправильное»;
- «это задание слишком сложное, дайте другое»;
- жалобы на то, что задание не соответствует уровню.

3) "task_evaluation"
Выбирается, если пользователь спрашивает о том, как оценивается его задание или какой балл он получил:
- «как оценивается задание?», «сколько баллов за это задание?»;
- «правильно ли я выполнил задание?», «как выставляется оценка?»;
- «почему я получил такую оценку?», «критерии оценки»;
- вопросы о системе оценивания, баллах, критериях.

4) "task_completion_status"
Выбирается, если пользователь спрашивает, выполнил ли он задание или засчиталось ли оно:
- «я выполнил задание?», «задание засчитано?»;
- «выполнил ли я задачу?», «сделал ли я это задание?»;
- «почему задание не засчиталось?», «не отмечено как выполненное»;
- вопросы о статусе выполнения конкретного задания.
...
SUBINTENT_PROMPTS = {
    ...
    IntentEnum.task_problems: subintent_task_problems_prompt,
}